...
use lsif_indexer::core::{
    ...
    IncrementalIndex, FileUpdate, calculate_file_hash,
};
...
fn create_small_graph() -> CodeGraph {
    ...
    // Add some edges
    if let (Some(idx0), Some(idx1)) = (graph.get_node_index("symbol_0"), graph.get_node_index("symbol_1")) {
        graph.add_edge(idx0, idx1, EdgeKind::Reference);
    }
    if let (Some(idx1), Some(idx2)) = (graph.get_node_index("symbol_1"), graph.get_node_index("symbol_2")) {
...
fn create_large_graph() -> CodeGraph {
    ...
    for i in 0..1000 {
        let symbol = Symbol {
            ...
            range: Range {
                start: Position { line: (i % 100) * 10, character: 0 },
                end: Position { line: (i % 100) * 10 + 5, character: 0 },
            },
            documentation: if i % 2 == 0 {
                Some(format!("Detailed documentation for entity_{} with various information", i))
            } else {
                None
            },
        };
...
fn benchmark_graph_construction(c: &mut Criterion) {
    ...
    group.bench_function("small_graph", |b| {
        b.iter(|| create_small_graph())
    });
    
    group.bench_function("medium_graph", |b| {
        b.iter(|| create_medium_graph())
    });
    
    group.bench_function("large_graph", |b| {
        b.iter(|| create_large_graph())
    });
...
fn benchmark_symbol_operations(c: &mut Criterion) {
    ...
    group.bench_function("find_symbol_small", |b| {
        b.iter(|| {
            small_graph.find_symbol(black_box("symbol_5"))
        })
    });
    
    group.bench_function("find_symbol_medium", |b| {
        b.iter(|| {
            medium_graph.find_symbol(black_box("symbol_50"))
        })
    });
    
    group.bench_function("find_symbol_large", |b| {
        b.iter(|| {
            large_graph.find_symbol(black_box("symbol_500"))
        })
    });
    
    group.bench_function("find_references_medium", |b| {
        b.iter(|| {
            medium_graph.find_references(black_box("symbol_50"))
        })
    });
    
    group.bench_function("find_references_large", |b| {
        b.iter(|| {
            large_graph.find_references(black_box("symbol_500"))
        })
    });
...
fn benchmark_lsif_operations(c: &mut Criterion) {
    ...
    group.bench_function("generate_lsif_small", |b| {
        b.iter(|| {
            generate_lsif(black_box(small_graph.clone())).unwrap()
        })
    });
    
    group.bench_function("generate_lsif_medium", |b| {
        b.iter(|| {
            generate_lsif(black_box(medium_graph.clone())).unwrap()
        })
    });
    
    group.bench_function("generate_lsif_large", |b| {
        b.iter(|| {
            generate_lsif(black_box(large_graph.clone())).unwrap()
        })
    });
    ...
    group.bench_function("parse_lsif_small", |b| {
        b.iter(|| {
            parse_lsif(black_box(&small_lsif)).unwrap()
        })
    });
    
    group.bench_function("parse_lsif_medium", |b| {
        b.iter(|| {
            parse_lsif(black_box(&medium_lsif)).unwrap()
        })
    });
    
    group.bench_function("parse_lsif_large", |b| {
        b.iter(|| {
            parse_lsif(black_box(&large_lsif)).unwrap()
        })
    });
...
fn benchmark_call_hierarchy(c: &mut Criterion) {
    ...
    group.bench_function("outgoing_calls_medium", |b| {
        ...
        b.iter(|| {
            analyzer.get_outgoing_calls(black_box("symbol_0"), black_box(3))
        })
    });
    ...
    group.bench_function("incoming_calls_medium", |b| {
        let analyzer = CallHierarchyAnalyzer::new(&medium_graph);
        b.iter(|| {
            analyzer.get_incoming_calls(black_box("symbol_50"), black_box(2))
        })
    });
    ...
    group.bench_function("incoming_calls_large", |b| {
        let analyzer = CallHierarchyAnalyzer::new(&large_graph);
        b.iter(|| {
            analyzer.get_incoming_calls(black_box("symbol_500"), black_box(2))
        })
    });
...
fn benchmark_incremental_updates(c: &mut Criterion) {
    ...
    for file_idx in 0..100 {
        ...
        for sym_idx in 0..10 {
            let symbol = Symbol {
                ...
                range: Range {
                    start: Position { line: sym_idx * 10, character: 0 },
                    end: Position { line: sym_idx * 10 + 5, character: 0 },
                },
    ...
    group.bench_function("dead_code_detection", |b| {
        ...
        for i in 0..20 {
            index_with_dead.add_symbol(Symbol {
...
fn benchmark_storage_operations(c: &mut Criterion) {
    ...
    group.bench_function("storage_full_save", |b| {
        ...
        b.iter(|| {
            storage.save_full(black_box(&index)).unwrap()
        })
    });
    ...
    group.bench_function("storage_incremental_save", |b| {
        ...
        b.iter(|| {
            storage.save_incremental(black_box(&index), black_box(&result)).unwrap()
        })
    });
