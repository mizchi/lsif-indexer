...
use lsif_indexer::core::parallel_optimized::{
    OptimizedParallelGraph, OptimizedParallelIndex,
};
...
fn generate_test_project(base_dir: &Path, num_files: usize, symbols_per_file: usize) {
    for file_idx in 0..num_files {
        ...
        // ファイル内容を生成
        content.push_str(&format!("// Module {} File {}\n\n", file_idx / 10, file_idx));
                ...
                if let Some(name_end) = line[name_start..].find('(').map(|i| i + name_start) {
                    ...
                    symbols.push(Symbol {
                        ...
                        range: Range {
                            ...
                            end: Position { line: line_no as u32 + 2, character: 0 },
                        },
...
fn benchmark_initial_indexing(c: &mut Criterion) {
    ...
    for (name, num_files, symbols_per_file) in test_cases {
        ...
        // 最適化並列版
        group.bench_function(
            BenchmarkId::new("parallel_optimized", name),
            |b| {
                b.iter_batched_ref(
                    || {
                        let temp_dir = TempDir::new().unwrap();
            ...
            || {
                ...
                let file_updates: Vec<_> = files
                    ...
                    .map(|file| {
                        let symbols = extract_symbols_from_file(file);
                        let hash = format!("hash_{}", file.to_string_lossy());
                        lsif_indexer::core::FileUpdate::Added {
                            path: file.clone(),
                            symbols,
                            hash,
                        }
            ...
            |(temp_dir, parallel_index, files)| {
                ...
                let file_updates: Vec<_> = (0..files_to_update)
                    ...
                    .map(|i| {
                        ...
                        lsif_indexer::core::FileUpdate::Modified {
                            path: file.clone(),
                            symbols,
                            hash,
                        }
