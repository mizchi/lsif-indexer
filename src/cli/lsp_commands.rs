...
impl LspCommand {
    pub async fn execute(&self) -> Result<()> {
        match &self.command {
            ...
            LspSubcommand::References { file, line, column, include_declaration } => {
                self.execute_references(file, *line, *column, *include_declaration).await
            }
            ...
            LspSubcommand::Complete { file, line, column, trigger_character } => {
                self.execute_complete(file, *line, *column, trigger_character.as_deref()).await
            }
            LspSubcommand::SignatureHelp { file, line, column } => {
                self.execute_signature_help(file, *line, *column).await
            }
            LspSubcommand::CodeActions { file, start_line, start_column, end_line, end_column } => {
                self.execute_code_actions(file, *start_line, *start_column, *end_line, *end_column).await
            }
            LspSubcommand::Format { file, tab_size, insert_spaces } => {
                self.execute_format(file, *tab_size, *insert_spaces).await
            }
            LspSubcommand::Rename { file, line, column, new_name } => {
                self.execute_rename(file, *line, *column, new_name).await
            }
            LspSubcommand::CallHierarchy { file, line, column, direction } => {
                self.execute_call_hierarchy(file, *line, *column, direction).await
            }
            LspSubcommand::TypeHierarchy { file, line, column, direction } => {
                self.execute_type_hierarchy(file, *line, *column, direction).await
            }
            LspSubcommand::Diagnostics { file, severity } => {
                self.execute_diagnostics(file, severity.as_deref()).await
            }
            LspSubcommand::InlayHints { file, start_line, end_line } => {
    ...
    async fn execute_references(&self, file: &str, line: u32, column: u32, include_declaration: bool) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
    ...
    async fn execute_workspace_symbols(&self, query: &str, limit: usize) -> Result<()> {
        ...
        if let Some(symbols) = result {
            ...
            for (i, symbol) in symbols.iter().enumerate() {
                println!("[{}] {} ({})", i + 1, symbol.name, self.symbol_kind_to_string(symbol.kind));
                println!("    Location: {}:{}:{}", 
                    symbol.location.uri,
    ...
    async fn execute_complete(&self, file: &str, line: u32, column: u32, trigger_character: Option<&str>) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
    ...
    async fn execute_signature_help(&self, file: &str, line: u32, column: u32) -> Result<()> {
        ...
        if let Some(help) = result {
            for (i, signature) in help.signatures.iter().enumerate() {
                let is_active = help.active_signature.map(|idx| idx as usize == i).unwrap_or(false);
                println!("{}[{}] {}", if is_active { ">" } else { " " }, i + 1, signature.label);
                ...
                if let Some(params) = &signature.parameters {
                    ...
                    for (j, param) in params.iter().enumerate() {
                        let is_active_param = help.active_parameter.map(|idx| idx as usize == j).unwrap_or(false);
                        println!("      {}[{}] {}", 
    ...
    async fn execute_code_actions(&self, file: &str, start_line: u32, start_column: u32, end_line: u32, end_column: u32) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
        ...
        if let Some(response) = result {
            let actions: Vec<CodeAction> = response.into_iter()
    ...
    async fn execute_rename(&self, file: &str, line: u32, column: u32, new_name: &str) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
    ...
    async fn execute_call_hierarchy(&self, file: &str, line: u32, column: u32, direction: &str) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
        ...
        if let Some(items) = items {
            ...
            for item in items {
                println!("Item: {} ({})", item.name, self.symbol_kind_to_string(item.kind));
                println!("Location: {}:{}:{}", 
                    item.uri,
                ...
                match direction {
                    "incoming" => {
                        ...
                        if let Some(calls) = calls {
                            ...
                            for call in calls {
                                println!("  <- {} ({})", call.from.name, self.symbol_kind_to_string(call.from.kind));
                                println!("     From: {}:{}:{}", 
                                    call.from.uri,
                    ...
                    "outgoing" => {
                        ...
                        if let Some(calls) = calls {
                            ...
                            for call in calls {
                                println!("  -> {} ({})", call.to.name, self.symbol_kind_to_string(call.to.kind));
                                println!("     To: {}:{}:{}", 
                                    call.to.uri,
    ...
    async fn execute_type_hierarchy(&self, file: &str, line: u32, column: u32, direction: &str) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
        ...
        if let Some(items) = items {
            ...
            for item in items {
                println!("Type: {} ({})", item.name, self.symbol_kind_to_string(item.kind));
                println!("Location: {}:{}:{}", 
                    item.uri,
                    item.range.start.line + 1,
                    item.range.start.character + 1
                ...
                match direction {
                    "supertypes" => {
                        let types = client.type_hierarchy_supertypes(TypeHierarchySupertypesParams {
                            item: item.clone(),
                            work_done_progress_params: WorkDoneProgressParams::default(),
                            partial_result_params: PartialResultParams::default(),
                        })?;
                        ...
                        if let Some(types) = types {
                            ...
                            for t in types {
                                println!("  <- {} ({})", t.name, self.symbol_kind_to_string(t.kind));
                                println!("     At: {}:{}:{}", 
                                    t.uri,
                    ...
                    "subtypes" => {
                        let types = client.type_hierarchy_subtypes(TypeHierarchySubtypesParams {
                            item: item.clone(),
                            work_done_progress_params: WorkDoneProgressParams::default(),
                            partial_result_params: PartialResultParams::default(),
                        })?;
    ...
    async fn execute_diagnostics(&self, file: &str, severity: Option<&str>) -> Result<()> {
        ...
        for (i, diagnostic) in filtered_diagnostics.iter().enumerate() {
            ...
            println!("    Severity: {:?}", diagnostic.severity.map(|s| match s {
    ...
    async fn execute_inlay_hints(&self, file: &str, start_line: Option<u32>, end_line: Option<u32>) -> Result<()> {
        let client = self.create_client(file)?;
        let uri = self.file_to_uri(file)?;
    ...
    fn create_client(&self, file: &str) -> Result<AdvancedLspClient> {
        let adapter = detect_language(file)
            .context("Failed to detect language for file")?;
        
        AdvancedLspClient::new(adapter)
    }
    ...
    fn analyze_symbol_dependencies(
        ...
        if !info.references.is_empty() {
            println!("\nSymbol: {} ({})", symbol.name, self.symbol_kind_to_string(symbol.kind));
