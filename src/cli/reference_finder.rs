...
use crate::cli::language_adapter::{LanguageAdapter, detect_language_adapter};
use anyhow::Result;
use regex::Regex;
use std::path::Path;
use walkdir::WalkDir;
        ...
        if let Some(adapter) = detect_language_adapter(&path.to_string_lossy()) {
            if adapter.is_source_file(path) {
                if let Ok(content) = std::fs::read_to_string(path) {
                    let refs = find_references_in_file(path, &content, target_name, target_kind, adapter.as_ref())?;
