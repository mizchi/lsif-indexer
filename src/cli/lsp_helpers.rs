...
impl LspClientHelpers {
    ...
    pub fn create_for_file(file_path: &Path) -> Result<GenericLspClient> {
        let adapter = detect_language(file_path.to_str().unwrap_or(""))
            .ok_or_else(|| anyhow::anyhow!("Failed to detect language for file: {:?}", file_path))?;
        GenericLspClient::new(adapter)
