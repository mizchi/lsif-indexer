...
impl GitDiffDetector {
    ...
    fn add_commit_diff_changes(
        ...
        let repo = self.repo.as_ref().unwrap();
        let last_commit = repo.revparse_single(last_commit_ref)
            .context("Failed to parse last commit reference")?;
        let last_tree = last_commit.peel_to_tree()
        ...
        let diff = repo.diff_tree_to_tree(Some(&last_tree), Some(&head_tree), Some(&mut diff_opts))
            .context("Failed to create diff")?;
        
        // 既に処理済みのパスを記録
        let processed_paths: HashSet<PathBuf> = changes.iter()
            .map(|c| c.path.clone())
            .collect();
        ...
        for component in path.components() {
            if let Some(name) = component.as_os_str().to_str() {
                if name == ".git" || name == "target" || name == "node_modules" 
                    || name == ".idea" || name == ".vscode" || name == "tmp" {
                    return true;
                }
            }
        }
