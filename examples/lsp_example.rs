...
fn main() {
    ...
    // ホバー情報の取得
    println!("1. ホバー情報の取得:");
    println!("   cargo run --bin lsif-indexer -- lsp hover --file src/lib.rs --line 10 --column 15");
    println!();
    
    // コード補完
    println!("2. コード補完:");
    println!("   cargo run --bin lsif-indexer -- lsp complete --file src/main.rs --line 20 --column 10");
    println!();
    ...
    println!("   cargo run --bin lsif-indexer -- lsp index-with-lsp --project . --output lsp_index.db");
}
