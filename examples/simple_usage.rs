use core::{CodeGraph, EdgeKind, Symbol, SymbolKind, Position, Range};
...
fn main() {
    ...
    let symbols = vec![
        ...
        Symbol {
            ...
            range: Range {
                ...
                end: Position { line: 22, character: 5 },
            },
    ...
    println!("✓ シンボル追加 ({}個): {:?}", symbols.len(), start.elapsed());
    ...
    if let (Some(&main_idx), Some(&add_idx)) = 
        (graph.symbol_index.get("main"), graph.symbol_index.get("add")) {
    ...
    // 5. 参照検索
    let start = Instant::now();
    let refs = graph.find_references("Calculator").unwrap_or_else(|_| vec![]);
    println!("✓ 参照検索: {}件 ({:?})", refs.len(), start.elapsed());

    // 6. 全シンボル取得
    let start = Instant::now();
    let all_symbols: Vec<_> = graph.get_all_symbols().collect();
    println!("✓ 全シンボル取得: {}件 ({:?})", all_symbols.len(), start.elapsed());
