use lsif_indexer::core::{
    IncrementalIndex, FileUpdate, Symbol, SymbolKind, Range, Position, EdgeKind,
    calculate_file_hash,
};
...
fn test_edge_consistency_after_symbol_removal() {
    ...
    // ファイルごとに正しく追加
    index.update_file(Path::new("file1.rs"), vec![symbol_a], "hash1".to_string()).unwrap();
    index.update_file(Path::new("file2.rs"), vec![symbol_b], "hash2".to_string()).unwrap();
    index.update_file(Path::new("file3.rs"), vec![symbol_c], "hash3".to_string()).unwrap();
    ...
    assert_eq!(a_refs.len(), 0, "References from A should be empty after B is removed");
...
fn test_rapid_sequential_updates() {
    ...
    for i in 0..10 {
        let symbols = vec![
            create_test_symbol(&format!("func_{}", i), &format!("function_{}", i), "rapid_update.rs"),
            create_test_symbol(&format!("var_{}", i), &format!("variable_{}", i), "rapid_update.rs"),
        ];
...
fn test_circular_reference_removal() {
    ...
    // ファイルごとに正しく追加
    index.update_file(Path::new("file_a.rs"), vec![symbol_a], "hash_a".to_string()).unwrap();
    index.update_file(Path::new("file_b.rs"), vec![symbol_b], "hash_b".to_string()).unwrap();
    index.update_file(Path::new("file_c.rs"), vec![symbol_c], "hash_c".to_string()).unwrap();
    ...
    for file_idx in 0..files_count {
        ...
        for sym_idx in 0..symbols_per_file {
            symbols.push(Symbol {
                ...
                range: Range {
                    start: Position { line: (sym_idx * 10) as u32, character: 0 },
                    end: Position { line: (sym_idx * 10 + 5) as u32, character: 0 },
                },
                documentation: None,
            });
        }
    ...
    for file_idx in 0..files_count {
        ...
        for sym_idx in 0..symbols_per_file/2 {
            new_symbols.push(Symbol {
                ...
                file_path: format!("file_{}.rs", file_idx),
                range: Range {
                    start: Position { line: ((sym_idx + symbols_per_file/2) * 10) as u32, character: 0 },
                    end: Position { line: ((sym_idx + symbols_per_file/2) * 10 + 5) as u32, character: 0 },
                },
                documentation: None,
            });
        }
...
fn test_mixed_update_operations() {
    ...
    for i in 0..5 {
        let symbols = vec![
            create_test_symbol(&format!("init_s{}_1", i), &format!("symbol_{}_1", i), &format!("file_{}.rs", i)),
            create_test_symbol(&format!("init_s{}_2", i), &format!("symbol_{}_2", i), &format!("file_{}.rs", i)),
        ];
        index.update_file(
    ...
    let mixed_updates = vec![
        ...
        FileUpdate::Added {
            path: PathBuf::from("file_4.rs"),
            symbols: vec![
                create_test_symbol("new_s4_1", "brand_new_symbol_4_1", "file_4.rs"),
            ],
        ...
        FileUpdate::Added {
            ...
            symbols: vec![
                create_test_symbol("new_s5_1", "new_file_symbol_5_1", "file_5.rs"),
            ],
    ...
    // ファイルメタデータの確認
    assert!(index.file_metadata.contains_key(&PathBuf::from("file_0.rs")));
    assert!(index.file_metadata.contains_key(&PathBuf::from("file_1.rs")));
    assert!(!index.file_metadata.contains_key(&PathBuf::from("file_2.rs")));
    assert!(index.file_metadata.contains_key(&PathBuf::from("file_3.rs")));
    assert!(index.file_metadata.contains_key(&PathBuf::from("file_4.rs")));
    assert!(index.file_metadata.contains_key(&PathBuf::from("file_5.rs")));
    
    // 各ファイルのシンボル数を確認
    assert_eq!(index.file_metadata[&PathBuf::from("file_0.rs")].symbols.len(), 2);
    assert_eq!(index.file_metadata[&PathBuf::from("file_1.rs")].symbols.len(), 2);
    assert_eq!(index.file_metadata[&PathBuf::from("file_3.rs")].symbols.len(), 2);
    assert_eq!(index.file_metadata[&PathBuf::from("file_4.rs")].symbols.len(), 1);
    assert_eq!(index.file_metadata[&PathBuf::from("file_5.rs")].symbols.len(), 1);
}
...
fn test_symbol_to_file_mapping_consistency() {
    ...
    assert_eq!(index.symbol_to_file["map_test_1"], PathBuf::from("original.rs"));
    ...
    // 古いファイルから削除
    index.update_file(Path::new("original.rs"), vec![], "hash_empty".to_string()).unwrap();
    
    // 新しいファイルに追加
    index.update_file(Path::new("moved.rs"), vec![symbol1_moved], "hash_moved".to_string()).unwrap();
    
    // マッピングが更新されていることを確認
    assert_eq!(index.symbol_to_file["map_test_1"], PathBuf::from("moved.rs"));
    assert_eq!(index.graph.find_symbol("map_test_1").unwrap().file_path, "moved.rs");
}
...
fn test_dead_code_detection_consistency() {
    ...
    // シンボルを追加（ファイルごとに）
    index.update_file(Path::new("main.rs"), vec![main], "hash_main".to_string()).unwrap();
    index.update_file(Path::new("lib.rs"), vec![lib_pub], "hash_lib".to_string()).unwrap();
    index.update_file(Path::new("utils.rs"), vec![util1, util2], "hash_utils".to_string()).unwrap();
    index.update_file(Path::new("internal.rs"), vec![internal], "hash_internal".to_string()).unwrap();
    index.update_file(Path::new("orphan.rs"), vec![orphan], "hash_orphan".to_string()).unwrap();
    ...
    // 依存関係を構築
    index.graph.add_edge(main_idx, util1_idx, EdgeKind::Reference);
    index.graph.add_edge(pub_idx, util2_idx, EdgeKind::Reference);
    index.graph.add_edge(util1_idx, internal_idx, EdgeKind::Reference);
    index.graph.add_edge(util2_idx, internal_idx, EdgeKind::Reference);
    ...
    index.update_file(Path::new("utils.rs"), empty_symbols, "hash_empty".to_string()).unwrap();
    
    // util1だけを再追加
    let util1_only = vec![create_test_symbol("util1", "utility1", "utils.rs")];
    index.update_file(Path::new("utils.rs"), util1_only, "hash_util1_only".to_string()).unwrap();
...
"#;
    ...
    index.update_file(&main_path, main_symbols, main_hash.clone()).unwrap();
    index.update_file(&utils_path, utils_symbols, utils_hash.clone()).unwrap();
    
    // エッジを追加（main -> helper）
    let main_idx = index.graph.get_node_index(&format!("{}:main", main_path.display())).unwrap();
    let helper_idx = index.graph.get_node_index(&format!("{}:helper", utils_path.display())).unwrap();
    index.graph.add_edge(main_idx, helper_idx, EdgeKind::Reference);
    ...
    assert_eq!(result.dead_symbols.len(), 1);
    assert!(result.dead_symbols.contains(&format!("{}:unused_internal", utils_path.display())));
...
"#;
    ...
    // 更新を実行
    let update_result = index.update_file(&utils_path, utils_symbols_v2, utils_hash_v2).unwrap();
    ...
    let helper_idx = index.graph.get_node_index(&format!("{}:helper", utils_path.display())).unwrap();
    let internal_helper_idx = index.graph.get_node_index(&format!("{}:internal_helper", utils_path.display())).unwrap();
    index.graph.add_edge(helper_idx, internal_helper_idx, EdgeKind::Reference);
    ...
    // unused_internalのみがデッドコード
    assert_eq!(result2.dead_symbols.len(), 1);
    assert!(result2.dead_symbols.contains(&format!("{}:unused_internal", utils_path.display())));
    assert!(!result2.dead_symbols.contains(&format!("{}:internal_helper", utils_path.display())));
}
