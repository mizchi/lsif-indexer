...
"#;
    ...
    // 基本的なシンボルが含まれているか
    assert!(symbol_names.iter().any(|n| n.contains("User")),
            "Should find User interface");
    assert!(symbol_names.iter().any(|n| n.contains("UserService")),
...
"#;
    ...
    assert!(symbol_names.iter().any(|n| n == "User"),
            "Should find User struct");
    assert!(symbol_names.iter().any(|n| n == "main"),
...
"#, i, i);
    ...
    assert!(duration.as_secs() < 1, "Symbol extraction should complete within 1 second");
