use lsif_indexer::core::{
    ...
    QueryParser, QueryEngine, format_query_results,
};
...
fn test_parse_simple_patterns() {
    ...
    assert_eq!(pattern.relationships[0].edge_type, Some(EdgeKind::Reference));
...
fn test_query_find_implementations() {
    ...
    let impl_names: Vec<String> = results.matches.iter()
...
fn test_query_find_references() {
    ...
    for match_result in &results.matches {
        let var_binding = match_result.bindings.iter()
            .find(|(v, _)| v == "var")
            .map(|(_, s)| &s.name);
        let cls_binding = match_result.bindings.iter()
            .find(|(v, _)| v == "cls")
            .map(|(_, s)| &s.name);
...
fn test_query_transitive_references() {
    ...
    let has_log_to_console = results.matches.iter().any(|m| {
        let fn_name = m.bindings.iter()
            .find(|(v, _)| v == "fn")
            .map(|(_, s)| &s.name);
        let cls_name = m.bindings.iter()
            .find(|(v, _)| v == "cls")
            .map(|(_, s)| &s.name);
...
fn test_query_with_paths() {
    ...
    let pattern = QueryParser::parse("(fn:Function)-[:Reference*1..3]->(interface:Interface)").unwrap();
    let results = engine.execute(&pattern);
