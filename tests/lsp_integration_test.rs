...
use lsif_indexer::cli::{
    ...
    advanced_lsp_features::*,
    lsp_adapter::{RustAnalyzerAdapter, TypeScriptAdapter, PythonAdapter, detect_language, LspAdapter},
...
fn test_lsp_client_basic() -> Result<()> {
    ...
    if test_file.exists() {
        ...
        // Test hover
        let hover = client.hover(uri.clone(), lsp_types::Position { line: 0, character: 0 })?;
...
fn test_language_detection() {
    ...
    assert!(detect_language("app.py").is_some());
...
"#)?;
    ...
    let function_symbols: Vec<_> = index.symbols.values()
...
fn test_lsp_cli_commands() -> Result<()> {
    ...
    assert!(help_text.contains("implementations"));
...
mod edge_case_tests {
    ...
    fn test_empty_file_handling() {
        ...
        if let Ok(client) = AdvancedLspClient::new(adapter) {
            ...
            let symbols = lsp_client.get_document_symbols(uri.as_str());
...
"#).unwrap();
    ...
    async fn test_multi_language_support() {
        ...
        // 各言語のアダプターをテスト
        for (file, adapter) in [
            (rust_file, Box::new(RustAnalyzerAdapter) as Box<dyn LspAdapter>),
            (ts_file, Box::new(TypeScriptAdapter) as Box<dyn LspAdapter>),
            (py_file, Box::new(PythonAdapter) as Box<dyn LspAdapter>),
        ] {
            if let Ok(client) = AdvancedLspClient::new(adapter) {
