use lsif_indexer::cli::git_diff::{GitDiffDetector, FileChangeStatus, FileChange};
...
fn test_detect_modified_files() -> Result<()> {
    ...
    // ファイルを変更（コミットせず）
    fs::write(temp_dir.path().join("test.rs"), "fn new() { println!(\"changed\"); }")?;
...
fn test_detect_deleted_files() -> Result<()> {
    ...
    // ファイルをコミット
    commit_file(&repo, "to_delete.rs", "fn delete_me() {}", "Add file to delete")?;
...
fn test_detect_changes_between_commits() -> Result<()> {
    ...
    let added = changes.iter().filter(|c| matches!(c.status, FileChangeStatus::Added)).count();
    let modified = changes.iter().filter(|c| matches!(c.status, FileChangeStatus::Modified)).count();
...
fn test_xxhash_performance() -> Result<()> {
    ...
    assert!(duration.as_millis() < 10, "Hash calculation took too long: {:?}", duration);
...
fn test_renamed_file_detection() -> Result<()> {
    ...
    assert!(changes.iter().any(|c| 
        matches!(c.status, FileChangeStatus::Modified) || 
