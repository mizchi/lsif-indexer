use lsif_indexer::core::incremental::{
    IncrementalIndex, FileUpdate, UpdateResult, calculate_file_hash
};
...
fn test_update_file_modify_symbols() {
    ...
    let updated_symbols = vec![
        create_test_symbol("func1", "function1_modified", SymbolKind::Function, "/src/lib.rs"),
...
fn test_remove_file() {
    ...
    assert!(result.removed_symbols.contains("func2"));
    assert_eq!(index.graph.symbol_count(), 0);
    assert!(!index.file_metadata.contains_key(&PathBuf::from("/src/lib.rs")));
}
...
fn test_dead_code_detection_simple() {
    ...
    let used_func = create_test_symbol("used_func", "used_function", SymbolKind::Function, "/src/lib.rs");
    let unused_func = create_test_symbol("unused_func", "unused_function", SymbolKind::Function, "/src/lib.rs");
...
fn test_dead_code_detection_with_public_api() {
    ...
    let internal_func = create_test_symbol("internal_func", "internal", SymbolKind::Function, "/src/lib.rs");
    let unused_func = create_test_symbol("unused_func", "unused", SymbolKind::Function, "/src/lib.rs");
    
    let pub_idx = index.graph.add_symbol(pub_func);
    let internal_idx = index.graph.add_symbol(internal_func);
    let _unused_idx = index.graph.add_symbol(unused_func);
    
    index.graph.add_edge(pub_idx, internal_idx, EdgeKind::Reference);
...
fn test_batch_update() {
    ...
    let updates = vec![
        ...
        FileUpdate::Added {
            ...
            symbols: vec![
                create_test_symbol("func3", "function3", SymbolKind::Function, "/src/file2.rs"),
            ],
        ...
        FileUpdate::Modified {
            path: PathBuf::from("/src/file1.rs"),
            symbols: vec![
                create_test_symbol("func1", "function1_modified", SymbolKind::Function, "/src/file1.rs"),
            ],
...
fn test_file_metadata() {
    ...
    let metadata = index.file_metadata.get(&PathBuf::from("/src/lib.rs")).unwrap();
