...
fn test_performance_metrics() {
    ...
    measure_language_performance("Python", Box::new(PythonAdapter::new()), "test-python-project");
    measure_language_performance("TypeScript", Box::new(TypeScriptAdapter::new()), "test-typescript-project");
...
fn measure_language_performance(
    ...
    println!("    • Symbol extraction:{:>8.2?} (total)", total_extraction_time);
    println!("    • Shutdown:         {:>8.2?}", shutdown_time);
    println!("    • Total time:       {:>8.2?}", 
    ...
    for (name, size, symbols, time) in &file_metrics {
        ...
        println!("    • {:<20} {:>6} bytes, {:>3} symbols, {:>6.2?} ({:.0} sym/s)",
...
fn test_memory_usage() {
    ...
    // 各言語でのメモリ使用量を測定
    let languages = vec![
        ("Go", Box::new(GoAdapter) as Box<dyn MinimalLanguageAdapter>, "test-go-project"),
        ("Python", Box::new(PythonAdapter::new()), "test-python-project"),
        ("TypeScript", Box::new(TypeScriptAdapter::new()), "test-typescript-project"),
    ];
