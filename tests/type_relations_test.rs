use lsif_indexer::core::{
    ...
    TypeRelationsAnalyzer, format_type_relations,
};
...
fn create_type_hierarchy_graph() -> CodeGraph {
    ...
    let admin_user = Symbol {
        ...
        range: Range {
            ...
            end: Position { line: 25, character: 30 },
        },
    ...
    let save_method = Symbol {
        ...
        range: Range {
            ...
            end: Position { line: 35, character: 5 },
        },
...
fn test_collect_type_relations_with_hierarchy() {
    ...
    let relations = analyzer.collect_type_relations("class:UserModel", 3).unwrap();
    ...
    assert!(relations.extensions.iter().any(|e| e.id == "class:AdminModel"));
...
fn test_group_relations_by_type() {
    ...
    println!("  Functions returning: {}", groups.functions_returning_type.len());
    ...
    assert!(groups.referenced_by.iter().any(|r| r.id == "fn:processContainer"));
}
...
fn test_type_relations_for_interface() {
    ...
    assert!(relations.extensions.iter().any(|i| i.id == "class:BaseModel"));
...
fn test_max_depth_limiting() {
    ...
    let relations_d0 = analyzer.collect_type_relations("class:BaseModel", 0).unwrap();
    
    // Test with depth 2 (recursive)
    let relations_d2 = analyzer.collect_type_relations("class:BaseModel", 2).unwrap();
