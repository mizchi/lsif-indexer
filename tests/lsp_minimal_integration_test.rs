...
fn test_gopls_with_minimal_client() {
    ...
    match client.initialize(&project_path) {
        Ok(result) => {
            ...
            println!("  Server name: {:?}", result.server_info.as_ref().map(|i| &i.name));
            println!("  Server version: {:?}", result.server_info.as_ref().and_then(|i| i.version.as_ref()));
        }
        Err(e) => {
    ...
    match client.get_document_symbols(&main_file) {
        Ok(symbols) => {
            ...
            // 特定のシンボルの存在確認
            assert!(symbols.iter().any(|s| s.name == "Calculator"), "Calculator type should exist");
            assert!(symbols.iter().any(|s| s.name == "main"), "main function should exist");
            assert!(symbols.iter().any(|s| s.name == "NewCalculator"), "NewCalculator function should exist");
        }
        Err(e) => {
    ...
    match client.get_document_symbols(&utils_file) {
        Ok(symbols) => {
            ...
            assert!(symbols.iter().any(|s| s.name == "StringUtils"), "StringUtils type should exist");
            assert!(symbols.iter().any(|s| s.name == "CountWords"), "CountWords function should exist");
        }
        Err(e) => {
    ...
    match client.find_references(&main_file, Position { line: 7, character: 5 }) {
        Ok(refs) => {
...
fn test_multiple_file_analysis() {
    ...
    assert!(total_symbols > 10, "Should find multiple symbols across files");
