use lsif_indexer::core::{
    ...
    DefinitionChainAnalyzer, format_definition_chain,
};
...
fn create_complex_graph() -> CodeGraph {
    ...
    let admin_var = Symbol {
        ...
        range: Range {
            ...
            end: Position { line: 20, character: 40 },
        },
...
fn test_shortest_definition_path() {
    ...
    let symbols: Vec<Symbol> = (0..6).map(|i| Symbol {
        id: format!("type:T{}", i),
        name: format!("Type{}", i),
        kind: SymbolKind::Class,
        file_path: "types.rs".to_string(),
        range: Range {
            start: Position { line: i * 5, character: 0 },
            end: Position { line: i * 5, character: 20 },
        },
        documentation: None,
    }).collect();
    ...
    let shortest = analyzer.get_shortest_definition_path("type:T0", "type:T5").unwrap();
