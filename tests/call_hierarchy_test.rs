...
fn test_call_hierarchy_with_sample_code() {
    ...
    use lsif_indexer::core::{CallHierarchyAnalyzer, format_hierarchy};
...
fn create_sample_graph() -> CodeGraph {
    ...
    for (name, kind, line) in symbols {
        let symbol = Symbol {
            ...
            range: Range {
                ...
                end: Position { line: line + 3, character: 0 },
            },
            documentation: None,
        };
    ...
    graph.add_edge(indices["main"], indices["DataProcessor::new"], EdgeKind::Reference);
    
    // calculate calls
    graph.add_edge(indices["calculate"], indices["add"], EdgeKind::Reference);
    graph.add_edge(indices["calculate"], indices["multiply"], EdgeKind::Reference);
    graph.add_edge(indices["calculate"], indices["combine"], EdgeKind::Reference);
    
    // DataProcessor::process calls
    graph.add_edge(indices["DataProcessor::process"], indices["DataProcessor::validate"], EdgeKind::Reference);
    graph.add_edge(indices["DataProcessor::process"], indices["DataProcessor::transform"], EdgeKind::Reference);
    graph.add_edge(indices["DataProcessor::process"], indices["DataProcessor::output"], EdgeKind::Reference);
    
    // DataProcessor::validate calls
    graph.add_edge(indices["DataProcessor::validate"], indices["DataProcessor::check_item"], EdgeKind::Reference);
    
    // DataProcessor::transform calls
    graph.add_edge(indices["DataProcessor::transform"], indices["DataProcessor::transform_item"], EdgeKind::Reference);
