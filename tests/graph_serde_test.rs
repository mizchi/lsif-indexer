...
fn test_round_trip_bincode() {
    ...
    let indices: Vec<_> = symbols.into_iter()
...
fn test_preserve_documentation() {
    ...
    assert_eq!(found.documentation, Some("This is important documentation".to_string()));
}
...
fn test_all_edge_kinds() {
    ...
    let symbols: Vec<_> = (0..9)
        .map(|i| create_test_symbol(&format!("s{}", i), &format!("symbol{}", i), SymbolKind::Function, "/test.rs"))
...
fn test_large_graph_performance() {
    ...
    let symbols: Vec<_> = (0..100)
        .map(|i| create_test_symbol(&format!("sym{}", i), &format!("symbol{}", i), SymbolKind::Function, "/large.rs"))
        .map(|s| graph.add_symbol(s))
        .collect();
    ...
    println!("Serialize time: {:?}, Deserialize time: {:?}", serialize_time, deserialize_time);
...
fn test_pretty_json_format() {
    ...
    let func = create_test_symbol("test_func", "test_function", SymbolKind::Function, "/test.rs");
