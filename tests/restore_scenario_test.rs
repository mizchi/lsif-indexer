...
fn test_git_rollback_and_restore() -> Result<()> {
    ...
    // バージョン1のコード
    let v1_commit = project.commit_file("main.rs", r#"
        fn main() {
            println!("Version 1");
        }
    ...
    let v2_commit = project.commit_file("main.rs", r#"
        fn main() {
            println!("Version 2");
        }
    ...
    // 差分インデックス
    let mut indexer = DifferentialIndexer::new(&project.db_path, project.temp_dir.path())?;
    let result2 = indexer.index_differential()?;
    ...
    assert_eq!(metadata_v1_restored.git_commit_hash, Some(v1_commit.to_string()));
...
fn test_content_hash_based_detection() -> Result<()> {
    ...
    // file1を変更（コミットなし）
    fs::write(project.temp_dir.path().join("file1.rs"), "fn func1_modified() {}")?;
    ...
    // ハッシュが変わったことを確認
    let old_hash = initial_hashes.get(&file1_path).or_else(|| {
        initial_hashes.iter()
...
fn test_mixed_git_and_untracked_files() -> Result<()> {
    ...
    // Git管理外のファイルを追加
    fs::write(project.temp_dir.path().join("untracked.rs"), "fn untracked() {}")?;
    ...
    assert!(metadata.file_hashes.iter().any(|(k, _)| k.ends_with("untracked.rs")));
    
    // untrackedファイルを変更
    fs::write(project.temp_dir.path().join("untracked.rs"), "fn untracked_modified() {}")?;
...
fn test_restore_after_branch_switch() -> Result<()> {
    ...
    // mainブランチでファイルを作成
    project.commit_file("main_feature.rs", r#"
        fn main_feature() {
    ...
    project.repo.branch("feature", &project.repo.find_commit(main_commit)?, false)?;
    
    // featureブランチにチェックアウト
    project.repo.set_head("refs/heads/feature")?;
    project.repo.checkout_head(Some(git2::build::CheckoutBuilder::new().force()))?;
    
    // featureブランチで異なるファイルを作成
    project.commit_file("feature_specific.rs", r#"
        fn feature_specific() {
