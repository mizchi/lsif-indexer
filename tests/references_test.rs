use lsif_indexer::cli::reference_finder::{find_all_references, Reference};
use lsif_indexer::core::{Symbol, SymbolKind, Position, Range};
use std::fs;
use std::path::Path;
...
fn test_find_function_references() {
    ...
    // 期待される参照数をチェック
    assert!(references.len() >= 3, "Expected at least 3 references, got {}", references.len());
    ...
    let main_refs: Vec<_> = references.iter()
        ...
        .collect();
    assert!(main_refs.len() >= 3, "Expected at least 3 references in main.rs");
}
...
fn test_find_struct_references() {
    ...
    // 期待される参照数をチェック
    assert!(references.len() >= 4, "Expected at least 4 references, got {}", references.len());
...
fn test_no_references_found() {
    ...
    assert!(references.is_empty(), "Expected no references for non-existent function");
}
...
"#;
    ...
    // 定義と有効な呼び出しのみが検出される
    assert_eq!(references.len(), 2, "Expected exactly 2 references (1 definition + 1 usage)");
