...
impl QueryEngine {
        ...
        while let Some((current, depth)) = queue.pop_front() {
            ...
            if rel.max_depth.is_none() || depth < rel.max_depth.unwrap() {
                let neighbors = match rel.direction {
                    Direction::Forward => self.graph.get_outgoing_edges(&current.id, rel.edge_type)?,
                    Direction::Backward => self.graph.get_incoming_edges(&current.id, rel.edge_type)?,
                    Direction::Both => {
                        let mut edges = self.graph.get_outgoing_edges(&current.id, rel.edge_type)?;
    ...
    pub fn parse_query(&self, query: &str) -> Result<QueryPattern> {
        ...
        Ok(QueryPattern { nodes, relationships })
    }
