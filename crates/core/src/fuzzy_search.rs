        ...
        if let Some(ids) = self.name_index.get(&query_lower) {
            for id in ids.iter() {
                if let Some(symbol) = self.symbols.get(id) {
                    results.insert(id.clone(), SearchResult {
                        symbol: symbol.clone(),
                        score: 100.0,
                    ...
                    if !results.contains_key(id) {
                        if let Some(symbol) = self.symbols.get(id) {
                            ...
                            if name_lower.starts_with(&query_lower) {
                                let score = 90.0 - (name_lower.len() - query_lower.len()) as f32 * 0.5;
                                results.insert(id.clone(), SearchResult {
                                    symbol: symbol.clone(),
                                    score,
    ...
    pub fn stats(&self) -> IndexStats {
        IndexStats {
            ...
            avg_symbols_per_bigram: if self.bigram_index.is_empty() {
                ...
                let total: usize = self.bigram_index.iter()
            ...
            avg_symbols_per_trigram: if self.trigram_index.is_empty() {
                ...
                let total: usize = self.trigram_index.iter()
                    .map(|entry| entry.value().len())
                    .sum();
...
mod tests {
    ...
    fn test_exact_match() {
        ...
        assert_eq!(matched.match_type, MatchType::Exact, "Should be exact match for case-insensitive search");
    ...
    fn test_short_query() {
        ...
        assert!(results.iter().any(|r| r.symbol.name.to_lowercase().starts_with("g")));
    }
    ...
    fn test_ranking_order() {
        ...
        let _get_user_by_id = results.iter().find(|r| r.symbol.name == "getUserById").unwrap();
