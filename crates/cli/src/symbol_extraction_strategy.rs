    ...
    pub fn extract(&self, path: &Path) -> Result<Vec<Symbol>> {
        for strategy in &self.strategies {
            if !strategy.supports(path) {
                debug!("Strategy '{}' does not support {}", strategy.name(), path.display());
                continue;
            }
            
            debug!("Trying strategy '{}' for {}", strategy.name(), path.display());
            
            match strategy.extract(path) {
                Ok(symbols) if !symbols.is_empty() => {
                    info!("Strategy '{}' extracted {} symbols from {}", 
                          strategy.name(), symbols.len(), path.display());
                    return Ok(symbols);
                }
                Ok(_) => {
                    debug!("Strategy '{}' returned no symbols for {}", 
                           strategy.name(), path.display());
                }
                Err(e) => {
                    warn!("Strategy '{}' failed for {}: {}", 
                          strategy.name(), path.display(), e);
                }
            }
        }
...
fn convert_lsp_symbols_to_core(lsp_symbols: &[lsp_types::DocumentSymbol], path: &Path) -> Vec<Symbol> {
...
mod tests {
    ...
    fn test_chained_extractor() {
        ...
        impl SymbolExtractionStrategy for DummyStrategy {
            ...
            fn supports(&self, _: &Path) -> bool { true }
            fn extract(&self, _: &Path) -> Result<Vec<Symbol>> { Ok(Vec::new()) }
            fn priority(&self) -> u32 { 50 }
        }
        
        let extractor = ChainedSymbolExtractor::new()
            .add_strategy(Box::new(DummyStrategy));
