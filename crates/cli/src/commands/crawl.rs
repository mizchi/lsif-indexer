...
pub fn handle_crawl(
    ...
    print_info(
        &format!("Starting smart crawl from {} files (depth: {}, max: {} files)", 
            start_files.len(), max_depth, max_files),
        "🕷️"
    );
    ...
    print_info(&format!("Average symbols per file: {:.1}", symbols_per_file), "📊");
