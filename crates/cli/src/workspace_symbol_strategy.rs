...
impl WorkspaceSymbolExtractionStrategy {
    ...
    pub fn extract_workspace_symbols(&self) -> Result<Vec<Symbol>> {
        info!("Extracting workspace symbols from: {}", self.project_root.display());
    ...
    fn query_workspace_symbols(
        ...
        client.send_request::<_, Option<Vec<lsp_types::SymbolInformation>>>(
            "workspace/symbol",
            params,
        )?
    ...
    fn convert_workspace_symbol_to_core(
        ...
        Some(Symbol {
            id: format!(
                "{}#{}:{}",
                file_path_str,
                ws_symbol.location.range.start.line,
                ws_symbol.name
            ),
    ...
    fn supports(&self, path: &Path) -> bool {
        ...
        if let Ok(processed) = self.processed_files.lock() {
            if processed.contains(path) {
                debug!("File already processed via workspace/symbol: {}", path.display());
                return false;
            }
        }
...
fn convert_document_symbols_to_core(lsp_symbols: &[lsp_types::DocumentSymbol], path: &Path) -> Vec<Symbol> {
    use lsif_core::{Position, Range};
    ...
    fn process_symbol(
        ...
        symbols.push(Symbol {
            id: format!("{}#{}:{}", file_path, symbol.selection_range.start.line, symbol.name),
...
impl HybridSymbolExtractionStrategy {
    pub fn new(
        ...
        let workspace_strategy = WorkspaceSymbolExtractionStrategy::new(
            lsp_pool.clone(),
            project_root.clone(),
        );
...
mod tests {
    ...
    fn test_symbol_kind_conversion() {
        ...
        assert!(matches!(convert_symbol_kind(LspKind::CLASS), SymbolKind::Class));
        assert!(matches!(convert_symbol_kind(LspKind::FUNCTION), SymbolKind::Function));
        assert!(matches!(convert_symbol_kind(LspKind::VARIABLE), SymbolKind::Variable));
        assert!(matches!(convert_symbol_kind(LspKind::METHOD), SymbolKind::Method));
        assert!(matches!(convert_symbol_kind(LspKind::INTERFACE), SymbolKind::Interface));
        assert!(matches!(convert_symbol_kind(LspKind::STRUCT), SymbolKind::Struct));
        assert!(matches!(convert_symbol_kind(LspKind::ENUM), SymbolKind::Enum));
    ...
    fn test_workspace_symbol_conversion() {
        use lsp_types::{SymbolInformation, SymbolKind, Location, Position, Range, Url};
