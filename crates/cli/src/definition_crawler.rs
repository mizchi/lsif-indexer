...
use anyhow::Result;
use lsif_core::{Symbol, SymbolKind, Range, Position};
...
impl DefinitionCrawler {
    pub fn new(storage_path: &Path, project_root: &Path, max_depth: u32, max_files: usize) -> Result<Self> {
    ...
    pub fn crawl_from_file(&mut self, start_file: &Path) -> Result<CrawlStats> {
        info!("Starting definition-based crawl from: {}", start_file.display());
        ...
        while let Some(target) = queue.pop() {
            ...
            if target.depth > self.max_depth {
                debug!("Skipping {} (depth {} > max {})", 
                    target.path.display(), target.depth, self.max_depth);
                continue;
            }
            ...
            match self.index_single_file(&target.path, target.depth) {
                Ok(file_stats) => {
                    ...
                    if stats.files_indexed % 10 == 0 {
                        info!("Progress: {} files indexed, {} symbols found", 
        ...
        info!("Crawl completed: {} files indexed, {} symbols found", 
            stats.files_indexed, stats.symbols_found);
    ...
    pub fn crawl_from_files(&mut self, start_files: Vec<PathBuf>) -> Result<CrawlStats> {
        info!("Starting definition-based crawl from {} files", start_files.len());
    ...
    fn index_single_file(&mut self, file_path: &Path, depth: u32) -> Result<FileIndexStats> {
        ...
        // LSPクライアントを取得または作成
        let client = self.lsp_pool.get_or_create_client(file_path, &self.project_root)?;
        ...
        for symbol in &symbols {
            // 定義元を取得
            if let Ok(definitions) = self.get_definitions(&mut client_guard, file_path, &symbol.range.start) {
            ...
            // 参照を取得（深さが浅い場合のみ）
            if depth < 2 {  // 参照は深さ2まで
                if let Ok(references) = self.get_references(&mut client_guard, file_path, &symbol.range.start) {
        ...
        debug!("Found {} symbols and {} related files", symbol_count, related_files.len());
    ...
    /// LSPを使ってシンボルを抽出
    fn extract_symbols(&self, client: &mut GenericLspClient, file_path: &Path) -> Result<Vec<Symbol>> {
    ...
    /// 定義元を取得
    fn get_definitions(&self, client: &mut GenericLspClient, file_path: &Path, position: &Position) -> Result<Vec<PathBuf>> {
    ...
    /// 参照を取得
    fn get_references(&self, client: &mut GenericLspClient, file_path: &Path, position: &Position) -> Result<Vec<PathBuf>> {
        let lsp_position = lsp_types::Position {
            line: position.line,
            character: position.character,
        };
    ...
    fn convert_symbol_kind(lsp_kind: lsp_types::SymbolKind) -> SymbolKind {
        match lsp_kind {
            ...
            lsp_types::SymbolKind::VARIABLE | lsp_types::SymbolKind::PROPERTY => SymbolKind::Variable,
