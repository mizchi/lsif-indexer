...
use lsp_types::{
    DocumentSymbol, ServerCapabilities, SymbolInformation,
    WorkspaceSymbolParams, WorkDoneProgressParams, PartialResultParams,
};
use std::path::Path;
use std::sync::Arc;
use std::sync::Mutex;
...
impl AutoSwitchingLspClient {
    ...
    pub fn new(adapter: Box<dyn LspAdapter>) -> Result<Self> {
        ...
        // サーバーの機能を取得（既に初期化済み）
        let capabilities = client.get_server_capabilities()
    ...
    pub fn get_all_symbols(&self, workspace_root: &str) -> Result<Vec<SymbolInfo>> {
        if self.supports_workspace_symbol {
            ...
            Err(anyhow!("Neither workspace/symbol nor documentSymbol is supported"))
        }
    }
    ...
    fn get_symbols_via_workspace(&self, _workspace_root: &str) -> Result<Vec<SymbolInfo>> {
        ...
        let symbols = client.send_request::<_, Option<Vec<SymbolInformation>>>(
            "workspace/symbol",
            params,
        )?
        ...
        Ok(symbols.into_iter().map(SymbolInfo::from_workspace).collect())
    }
    ...
    fn should_skip_directory(&self, path: &Path) -> bool {
        ...
        // 一般的な除外パターン
        matches!(name, "node_modules" | ".git" | "target" | "dist" | "build" | ".vscode")
    }
    ...
    fn is_source_file(&self, path: &Path) -> bool {
        ...
        matches!(extension, "rs" | "ts" | "tsx" | "js" | "jsx" | "go" | "py" | "java" | "cpp" | "c" | "h")
    }
    ...
    pub fn get_file_symbols(&self, file_uri: &str) -> Result<Vec<SymbolInfo>> {
        if self.supports_document_symbol {
            ...
            Ok(symbols.into_iter().map(|s| SymbolInfo::from_document(s, file_uri)).collect())
        } else {
...
mod tests {
    ...
    use crate::test_utils::{
        MockLspAdapter, create_test_workspace_symbol, create_test_document_symbol,
    };
    ...
    fn test_symbol_info_conversion() {
        ...
        let ws_symbol = create_test_workspace_symbol(
            "TestClass",
            SymbolKind::CLASS,
            "/test/file.ts",
            20,
        ...
        let doc_symbol = create_test_document_symbol(
            "TestMethod",
            SymbolKind::METHOD,
            30,
            35,
    ...
    fn test_should_skip_directory() {
        ...
        for dir in &dirs_to_skip {
            ...
            let name = path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("");
            assert!(matches!(name, "node_modules" | ".git" | "target" | "dist" | "build" | ".vscode"));
        }
    }
    
    #[test]
    fn test_is_source_file() {
        let source_extensions = ["rs", "ts", "tsx", "js", "jsx", "go", "py", "java", "cpp", "c", "h"];
    ...
    fn test_with_real_tsgo() {
        ...
        // 機能を確認
        println!("Supports workspace/symbol: {}", client.has_workspace_symbol());
