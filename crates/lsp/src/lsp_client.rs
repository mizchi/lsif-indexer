...
impl LspClient {
    ...
    pub fn type_definition(&self, uri: Url, position: Position) -> Result<Vec<Location>> {
        ...
        match response {
            Some(lsp_types::request::GotoTypeDefinitionResponse::Scalar(location)) => {
                Ok(vec![location])
            }
            Some(lsp_types::request::GotoTypeDefinitionResponse::Array(locations)) => Ok(locations),
            Some(lsp_types::request::GotoTypeDefinitionResponse::Link(links)) => Ok(links
                .into_iter()
                .map(|link| Location {
                    uri: link.target_uri,
    ...
    pub fn implementation(&self, uri: Url, position: Position) -> Result<Vec<Location>> {
        ...
        match response {
            ...
            Some(lsp_types::request::GotoImplementationResponse::Link(links)) => Ok(links
                .into_iter()
                .map(|link| Location {
                    uri: link.target_uri,
                    range: link.target_selection_range,
