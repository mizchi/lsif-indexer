...
impl UnifiedIndexer {
    ...
    pub async fn index_project(&mut self, project_root: &Path) -> Result<IndexResult> {
        info!("Starting unified LSP-based indexing for: {}", project_root.display());
        ...
        info!("Indexing completed: {} files, {} symbols in {:.2}s",
              total_files, total_symbols, duration.as_secs_f64());
    ...
    fn convert_document_symbols(&self, doc_symbols: &[DocumentSymbol], file_uri: &str, file_path: &Path) -> Vec<Symbol> {
    ...
    pub async fn search_symbols(&self, query: &str, workspace_root: &Path) -> Result<Vec<Symbol>> {
        let symbol_infos = self.lsp_manager.search_workspace_symbols(workspace_root, query).await?;
        
        let mut symbols = Vec::new();
        for info in symbol_infos {
            // URIからファイルパスを取得
            let file_path = info.location.uri.to_file_path()
    ...
    async fn test_document_symbol_conversion() {
        ...
        let doc_symbol = DocumentSymbol {
            ...
            selection_range: lsp_types::Range {
                start: lsp_types::Position { line: 10, character: 5 },
                end: lsp_types::Position { line: 10, character: 18 },
            },
            children: None,
        };
        ...
        assert_eq!(symbol.documentation, Some("Test function detail".to_string()));
    }
