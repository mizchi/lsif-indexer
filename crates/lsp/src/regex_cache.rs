...
/// グローバルな正規表現キャッシュ
static REGEX_CACHE: Lazy<RwLock<RegexCache>> = Lazy::new(|| {
    RwLock::new(RegexCache::new())
});
...
impl RegexCache {
    ...
    /// 動的にパターンを追加
    pub fn add_pattern(&mut self, lang: Language, pattern_type: PatternType, pattern: &str) -> Result<(), regex::Error> {
...
pub fn add_regex_pattern(lang: Language, pattern_type: PatternType, pattern: &str) -> Result<(), regex::Error> {
    REGEX_CACHE
        .write()
...
mod tests {
    ...
    fn test_precompiled_patterns() {
        ...
        assert!(cache.get(Language::TypeScript, PatternType::Function).is_some());
        assert!(cache.get(Language::TypeScript, PatternType::Class).is_some());
        assert!(cache.get(Language::TypeScript, PatternType::Interface).is_some());
    }
    ...
    fn test_performance_improvement() {
        ...
        assert!(with_cache < without_cache / 10, 
