...
impl Default for LspServerRegistry {
    fn default() -> Self {
        ...
        // Java
        configs.insert("java".to_string(), LspServerConfig {
        ...
        // C/C++
        configs.insert("cpp".to_string(), LspServerConfig {
...
impl UnifiedLspManager {
    ...
    pub async fn get_client_for_file(&self, file_path: &Path) -> Result<Arc<RwLock<LspRpcClient>>> {
        ...
        self.get_or_create_client(&language, file_path.parent().unwrap_or(Path::new("."))).await
    }
    
    /// 言語に対応するLSPクライアントを取得または作成
    pub async fn get_or_create_client(&self, language: &str, workspace_root: &Path) -> Result<Arc<RwLock<LspRpcClient>>> {
        // 既存のクライアントを確認
    ...
    /// LSPクライアントを作成して初期化
    async fn create_lsp_client(&self, config: &LspServerConfig, workspace_root: &Path) -> Result<LspRpcClient> {
        info!("Starting LSP server for {}: {}", config.language_id, config.command);
        
        // Create LSP RPC client
        let mut client = LspRpcClient::new(&config.command, &config.args, config.language_id.clone())?;
        
        // Initialize with workspace root
        let root_uri = Url::from_file_path(workspace_root)
            .map_err(|_| anyhow::anyhow!("Invalid workspace root path"))?;
        
        client.initialize(root_uri, config.initialization_options.clone()).await?;
    ...
    /// ワークスペースシンボルを検索
    pub async fn search_workspace_symbols(&self, _workspace_root: &Path, query: &str) -> Result<Vec<SymbolInformation>> {
        ...
        for entry in self.clients.iter() {
            ...
            match client.workspace_symbols(query).await {
                ...
                Err(e) => warn!("Failed to get workspace symbols from {}: {}", entry.key(), e),
            }
        }
    ...
    pub async fn index_project(&self, project_root: &Path) -> Result<ProjectIndex> {
        ...
        let symbols_per_file: Vec<_> = files
            ...
            .filter_map(|file_path| {
                ...
                match tokio::runtime::Handle::current().block_on(
                    self.get_document_symbols(file_path)
                ) {
        ...
        info!("Indexed {} files with {} total symbols", 
